...
var GoogleSpreadsheet = function(ss_key, auth_id, options) {
  ...
  // This method is used internally to make all requests1
  ...
  this.addWorksheet = function(opts, cb) {
    ...
    self.makeFeedRequest(['worksheets', ss_key], 'POST', data_xml, function(
      err,
      data
    ) {
      if (err) return cb(err);
  ...
  this.getRows = function(worksheet_id, opts, cb) {
    ...
    self.makeFeedRequest(['list', ss_key, worksheet_id], 'GET', query, function(
      err,
      data,
      xml
    ) {
  ...
  this.addRow = function(worksheet_id, data, cb) {
    ...
    data_xml += '</entry>';
    self.makeFeedRequest(
      ['list', ss_key, worksheet_id],
      'POST',
      data_xml,
      function(err, data, new_xml) {
        if (err) return cb(err);
    ...
    self.makeFeedRequest(
      ...
      function(err, data) {
        ...
        while (entries.length > 0) {
          cells.push(
            new SpreadsheetCell(self, ss_key, worksheet_id, entries.shift())
          );
        }
...
var SpreadsheetWorksheet = function(spreadsheet, data) {
  ...
  self['_links']['cells'] =
    self['_links']['http://schemas.google.com/spreadsheets/2006#cellsfeed'];
  ...
  function _setInfo(opts, cb) {
    ...
    var xml =
      ...
      '</gs:colCount>' +
      '</entry>';
    spreadsheet.makeFeedRequest(self['_links']['edit'], 'PUT', xml, function(
      err,
      response
    ) {
      if (err) return cb(err);
  ...
  this.bulkUpdateCells = function(cells, cb) {
    ...
    spreadsheet.makeFeedRequest(
      self['_links']['bulkcells'],
      'POST',
      data_xml,
      function(err, data) {
        if (err) return cb(err);
        ...
        if (data.entry && data.entry.length)
          data.entry.forEach(function(cell_data) {
            cells_by_batch_id[
              cell_data['batch:id']
            ].updateValuesFromResponseData(cell_data);
          });
        cb();
      }
    );
...
function SpreadsheetCell(spreadsheet, ss_key, worksheet_id, data) {
  ...
  for (var i = 0; i < links.length; i++) {
    var link = links[i];
    if (link['$']['rel'] == 'self' && link['$']['href'] == this.getSelf())
      continue;
    if (link['$']['rel'] == 'edit' && link['$']['href'] == this.getEdit())
      continue;
...
SpreadsheetCell.prototype.save = function(cb) {
  ...
  this.spreadsheet.makeFeedRequest(this.getEdit(), 'PUT', data_xml, function(
    err,
    response
  ) {
    if (err) return cb(err);
